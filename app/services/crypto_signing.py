...
def verify_certificate_signature(
    ...
    if certificate["signature_algorithm"] != "Ed25519":
        return False
    if metadata_hash(metadata) != certificate["metadata_hash"]:
        return False
    if certificate_hash(certificate) != certificate["certificate_hash"]:
        return False
    try:
        public_key = load_public_key(certificate["public_key"])
        signature = base64.b64decode(certificate["signature"])
        public_key.verify(signature, certificate["certificate_hash"].encode("utf-8"))
        return True
    except Exception:
        return False
...
def get_trust_signing_material(
    ...
    try:
        ...
        raise RuntimeError("Production certificate signing key material is invalid.") from exc

    if not hmac.compare_digest(derived_public, public_key_b64):
        raise RuntimeError("Production certificate signing public/private keys do not match.")
...
def get_or_create_dev_trust_keypair(
    ...
    """Historical ingest hook, now returning the authoritative key ID too."""
