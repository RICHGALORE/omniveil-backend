...
def test_10_reanalysis_updates_without_duplicate():
    with TestClient(main.app, raise_server_exceptions=False) as client:
        ...
        # Re-upload identical content -> deterministic same omni_id and the
        # existing registration is reused while metadata is re-analyzed in place.
        up2 = _upload(client, RAW_KEY_A, "cam.jpg", content, "image/jpeg")
        assert up2.status_code == 200, up2.text
        assert up2.json()["omni_id"] == omni_id
        assert up2.json()["registration_reused"] is True
...
_EXPECTED_UPLOAD_KEYS = {
    ...
    "section_c_ownership_splits", "legal_disclaimer", "registration_reused",
}
...
def test_17_live_split_contributor_persistence_still_works():
    ...
    with TestClient(main.app, raise_server_exceptions=False) as client:
        ...
        # Use a unique byte identity so this test exercises first registration,
        # not the intentional idempotent replay path from the earlier MP3 test.
        live_split_bytes = _mp3_bytes() + b"\nOV-LIVE-SPLIT-17"
        up = _upload(client, RAW_KEY_A, "song-live-split.mp3", live_split_bytes,
                     "audio/mpeg", provenance=provenance)
        assert up.status_code == 200, up.text
        body = up.json()
